  ...
  const handleCancelAppt = async () => {
    try {
      const response = await axios.patch(
        `${process.env.REACT_APP_SERVER_URL}/appointments/updateApptStatus/${selectedAppt._id}`,
        {
          status: 'Cancelled',
        }
      );
      ...
      const remRes = await axios.patch(
        `${process.env.REACT_APP_SERVER_URL}/appointments/removeDate/${selectedAppt._id}/${selectedAppt.preferredDate}`,
