    ...
    const fetchData = async () => {
      try {
        const appointmentResponse = await axios.get(
          `http://localhost:4000/appointments/appointmentAndDoctor`
        );
          ...
          <div key={appointments._id} className="appt-cells">
            <div className="appt-cell-one">
              ...
              <p>
                ...
                {new Date(appointments.dateStart).toDateString()} -{' '}
                {new Date(appointments.dateEnd).toDateString()}{' '}
              </p>
              <p>
                {' '}
            ...
            <div className="appt-cell-two">
              <p>
                {' '}
                <span className="text-red-500">Patient Name:</span>{' '}
                {appointments.patientFirstName} {appointments.patientLastName}{' '}
              </p>
              <p>
                {' '}
                <span className="text-red-500">Doctor Name:</span>{' '}
                {appointments.doctorAppointment[0]?.fname}{' '}
                {appointments.doctorAppointment[0]?.lname}
              </p>
              <p>
                {' '}
            ...
            <div className="appt-cell-four">
              <p>
                ...
                {appointments.confirmed ? 'Confirmed' : 'Pending'}{' '}
              </p>
              <button className="appt-update-btn">Update Status</button>{' '}
              <br></br>
